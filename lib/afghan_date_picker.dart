...
import 'package:afghan_date_picker/utils/afghani.dart';
      ...
      if (gregorianDatetime != null &&
        ...
        if (gregorianFinishDate != null) {
          controller.text = translate(
                  outputFormat, gregorianDate.toAfghani(), false, locale) +
              rangeSeparator +
              translate(
                  outputFormat, gregorianFinishDate.toAfghani(), false, locale);
        } else {
          controller.text =
              translate(outputFormat, gregorianDate.toAfghani(), false, locale);
        }
      } else {
//        jDatetime = Afghani.now();
        ...
        if (datetime != null && datetime.toString().isNotEmpty) {
          ...
          if (jFinishDatetime != null) {
            controller.text = translate(outputFormat, jDatetime, true, locale) +
                rangeSeparator +
                translate(outputFormat, jFinishDatetime, true, locale);
          } else {
            controller.text = translate(outputFormat, jDatetime, true, locale);
          }
        }
      ...
      return AfghanDatePickerWidget(options: options);
    }
  }
}

class AfghanDatePickerWidget extends StatefulWidget {
  final Map<String, dynamic> options;

  AfghanDatePickerWidget({
    this.options,
  });
  ...
  @override
  _AfghanDatePickerWidgetState createState() => _AfghanDatePickerWidgetState();
}
...
class _AfghanDatePickerWidgetState extends State<AfghanDatePickerWidget>
    with TickerProviderStateMixin {
  final Afghani currentDate = Afghani.now();
  final Afghani startDate =
      Afghani.fromDateTime(DateTime(1925, 3, 21)); // 1304/1/1
  final List<Map<String, dynamic>> pages = [];
  final List<int> months = [];
  final List<int> years = [];

  Afghani inputSelectedDate;
  Afghani rangeStartDate;
  Afghani rangeFinishDate;
  ...
  inputStringToAfghani() {
    ...
    inputSelectedDate = Afghani.now();
    inputSelectedDateByUser = false;

    Afghani afghaniMinDatetime, afghaniMaxDatetime;
    if (widget.options['minDatetime'] != null) {
      afghaniMinDatetime = _parseDatetimeInput(widget.options['minDatetime'],
          widget.options['defaultInputFormat'], "/");
      standardMinDatetime = afghaniMinDatetime.toDateTime();
    }
    if (widget.options['maxDatetime'] != null) {
      afghaniMaxDatetime = _parseDatetimeInput(widget.options['maxDatetime'],
          widget.options['defaultInputFormat'], "/");
      standardMaxDatetime = afghaniMaxDatetime.toDateTime();
    }
    ...
    if (controllerTextTmp != null && controllerTextTmp.toString().isNotEmpty) {
      ...
      if (widget.options['rangeDatePicker'] == true) {
        ...
        if (standardMinDatetime != null) {
          if (standardStartDatetime.isBefore(standardMinDatetime)) {
            inputSelectedDate = afghaniMinDatetime;
          }
        }
        if (standardMaxDatetime != null) {
          if (standardStartDatetime.isAfter(standardMaxDatetime)) {
            inputSelectedDate = afghaniMaxDatetime;
          }
        }
      }
    }
...
// set current month and year of the datepicker to selected input string
    ...
    for (int year = startDate.year; year <= (currentDate.year + 100); year++) {
      ...
      for (int month = 1; month <= 12; month++) {
        ...
        int shamsiDaysOfMonth = Afghani(year, month).monthLength;
...
// if this row is the last row
                ...
                positionedYMSelection(), // years dialog widget
              ],
            ),
          ),
        ),
  ...
  Widget _dayBlock({int day, bool otherMonthDay = false, pageInfo}) {
    ...
    Afghani dayBlockDateTime =
        Afghani(datePickerCurrentYear, datePickerCurrentMonth, day);
...
//          BorderSide borderTop = BorderSide(color: widget.options['daysBorderColor'], width: widget.options['daysBorderWidth']);
            ...
            if (otherMonthDay == false && disabledDay == false) {
              ...
              if (widget.options['rangeDatePicker'] == true) {
                ...
                if (rangeStartDate == null ||
                  ...
                  rangeStartDate = Afghani(pageInfo['Y'], pageInfo['M'], day);
                  ...
                  rangeFinishDate = Afghani(pageInfo['Y'], pageInfo['M'], day);
                ...
                Afghani jd = Afghani(pageInfo['Y'], pageInfo['M'], day);
                widget.options['controller'].text = translate(
                    widget.options['outputFormat'],
                    jd,
                    widget.options['farsiDigits'],
  ...
  Widget _datePickerPageHeader(pageInfo) {
    Afghani datetime = Afghani(pageInfo['Y'], pageInfo['M'], 1);
  ...
  Widget displayYears() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        Afghani datetime = Afghani(years[index], datePickerCurrentMonth, 1);
  ...
  Widget displayMonths() {
    ...
    return ListView.builder(
      ...
      itemCount: 12,
      itemBuilder: (BuildContext context, int index) {
        Afghani datetime = Afghani(inputSelectedDate.year, index + 1, 1);
              ...
              children: <Widget>[
                Positioned(
                  ...
                  child: Center(
                    child: Text(
                      translate(
                          'MMM', datetime, false, widget.options['locale']),
                      style: textStyle,
                    ),
                  ),
                ),
