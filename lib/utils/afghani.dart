...
import 'package:afghan_date_picker/AfghanDatePickerLocale.dart';
...
class Afghani {
  ...
  int get julianDayNumber {
    final r = _AfghaniBreaks.cal(year);
  ...
  Afghani(this.year, [this.month = 1, this.day = 1]);

  factory Afghani.fromJulianDayNumber(julianDayNumber) {
    // Calculate Gregorian year (gy).
    int gy = Gregorian.fromJulianDayNumber(julianDayNumber).year;
    int jy = gy - 621;
    final r = _AfghaniBreaks.cal(jy);
    ...
    if (k >= 0) {
      if (k <= 185) {
        ...
        return Afghani(jy, jm, jd);
      } else {
        // The remaining months.
        k -= 186;
      }
    } else {
      // Previous Afghani year.
      jy -= 1;
      k += 179;
      if (r.leap == 1) k += 1;
    }
  ...
  factory Afghani.fromDateTime(DateTime dateTime) {
    return Gregorian.fromDateTime(dateTime).toAfghani();
  }

  /// Get Afghani date for now
  factory Afghani.now() {
    return Afghani.fromDateTime(DateTime.now());
  }

  /// Converts Afghani date to [DateTime] object
  DateTime toDateTime() {
    return toGregorian().toDateTime();
  }

  /// Converts a Afghani date to Gregorian.
  Gregorian toGregorian() {
    return Gregorian.fromJulianDayNumber(julianDayNumber);
  }

  /// Checks whether a Afghani date is valid or not.
  bool isValid() {
    return year >= -61 &&
        year <= 3177 &&
        month >= 1 &&
  ...
  bool isLeapYear() {
    return _AfghaniBreaks.cal(year).leap == 0;
  }

  /// Computes number of days in a given month in a Afghani year.
  int get monthLength {
...
class Gregorian {
  ...
  /// Converts a Gregorian date to Afghani.
  Afghani toAfghani() {
    return Afghani.fromJulianDayNumber(julianDayNumber);
  }
...
/// Internal class
class _AfghaniBreaks {
  /// Number of years since the last leap year (0 to 4)
  final int leap;

  /// Gregorian year of the beginning of Afghani year
  final int gy;

  /// The March day of Farvardin the 1st (1st day of jy)
  final int march;

  _AfghaniBreaks({this.leap, this.gy, this.march});

  /// This determines if the Afghani (Persian) year is
  /// leap (366-day long) or is the common year (365 days), and
  /// finds the day in March (Gregorian calendar) of the first
  /// day of the Afghani year (jy).
  ///
  /// [1. see here](http://www.astro.uni.torun.pl/~kb/Papers/EMP/PersianC-EMP.htm)
  ///
  /// [2. see here](http://www.fourmilab.ch/documents/calendar/)
  factory _AfghaniBreaks.cal(int jy) {
    // Afghani years starting the 33-year rule.
    final breaks = [
      -61,
      9,
      38,
    ...
    if (jy < jp || jy >= breaks[bl - 1]) throw 'Invalid Afghani year $jy';

    // Find the limiting years for the Afghani year jy.
    for (i = 1; i < bl; i += 1) {
      jm = breaks[i];
      jump = jm - jp;
      if (jy < jm) break;
    ...
    // of the current Afghani year in the Persian calendar.
    ...
    return _AfghaniBreaks(leap: leap, gy: gy, march: march);
  }
}
...
String translate(String input, Afghani date, bool farsiNum, AfghanDatePickerLocale locale) {
  ...
  input = input.replaceAll("MMM", getAfghaniMonthName(date.month, locale));
